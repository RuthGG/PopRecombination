...
# # Example
# args[1]<-"analysis/20220513_LocationPatterns/divideChromosomes/avgBherer_fixedArms_5/" # Directory for divideChromosomes
# args[2]<-"analysis/20220513_LocationPatterns/fillWindowMeasurements/avgBherer_fixedArms_5/"  # Directory with fillWindowMeasurements
# args[3]<-"analysis/20220519_LocationPatterns" # Exit file
...
divChroms<- args[1]
winMeasur<- args[2]
dir.create(paste0(args[3], "/statAnalysis_density/"))
outdir<- paste0(args[3], "/statAnalysis_density/",args[4])
dir.create(outdir)
name<-args[4]
...
visVars <- function(windowDatafile, name){
  ...
  windowData$Chromosome <- factor(windowData$Chromosome, levels = paste(rep("chr", 24), c(as.character(c(1:22)),"X", "Y"), sep = ""))
  windowData$chromID <- windowData$winID <- NULL
  
  # Make inversion density
  windowData$invDensity  = windowData$invCenters / windowData$Length.bp.
  windowData$NHDensity  = windowData$NHCenters / windowData$Length.bp.
  windowData$NAHRDensity  = windowData$NAHRCenters / windowData$Length.bp.
  ...
  windowDataMelted$dataGroup<- ifelse(windowDataMelted$variable %in% c("invCenters", "NHCenters", "NAHRCenters"), "Inversions", 
                                      ifelse(windowDataMelted$variable %in% c("invDensity", "NHDensity", "NAHRDensity"), "Inversion densities", 
...
# Setup
vars = c("invDensity", "NHDensity", "NAHRDensity")
figscale = 2
