...
# """.format(index, total))
   ...
   # winRegions.loc[index,"intraRepCounts"] = len(rep_part_A[(rep_part_A.chrom == rep_part_A.otherChrom)].index) + len(rep_part_B[(rep_part_B.chrom == rep_part_B.otherChrom)].index)
    ...
    # Recombination rates
    recRate_part = recRate[(recRate.Chromosome == row["Chromosome"]) & (recRate.Start <= row["End"]) & (recRate.End >= row["Start"])] # Take coords non-strictly
    if len(recRate_part.index) > 0:
        recRate_part.Start[recRate_part.Start == recRate_part.Start.min()] = row.Start # Cut coords to only the window
        recRate_part.End[recRate_part.End == recRate_part.End.max()] = row.End
        recRate_part["winSize"] = recRate_part.End - recRate_part.Start # Make window size

        winRegions.loc[index,"WAvgRate"] = np.average (recRate_part["Rate"], weights = recRate_part["winSize"] ) # Make average weighted by overlap with window, after adapting coords
