...
gatresdir<-"analysis/defTest_LocationPatterns/GATresults_doublesides/"
```
...
for (condition in conditions){
  gatResultMerged <- read.table(paste0("analysis/",date,"_LocationPatterns/GATresults_doublesides/",desc,"/",condition,".tsv"), sep = "\t", header = T)
  ...
  plot2<- ggplot(gatResultMerged)+
    ...
    facet_wrap(annotation  ~ ., scales = "free")+
     theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
      ...
      ggsave(paste0("analysis/",date,"_LocationPatterns/GATresults_doublesides/",desc,"/",condition,".png"),  grid.arrange(plot1, plot2), width=2480, height=3508, units = "px" , scale = 1.5)
}
}
```
...
OUTDIR=analysis/${TODAY}_LocationPatterns/GATresults_doublesides/${DESC}
...
for SEGMENT in $SEGMENTS; do
  for NAME in $CONDITIONS; do
     ...
     --num-samples=50000 --log=$OUTDIR/log/${SEGMENT}_${NAME}.log  --stdout=$OUTDIR/tsv/${SEGMENT}_${NAME}.tsv
...
```{r}
names<-c( "NHinvs_doublesides", "NHinvs_doublesides_iso", "NAHRinvs_doublesides","NAHRinvs_doublesides_iso")
...
for (n in names){for (feature in conditions){
  subtab<-read.table(paste0("analysis/",today,"_LocationPatterns/GATresults_doublesides/",n, "/", feature, ".tsv"), header = T)
...
ggsave( paste0("analysis/",today,"_LocationPatterns/GATresults_doublesides/GeneralResult.png"), ggarrange(c, d, ncol=1), width=2480, height=3508, units = "px" , scale = 1)
