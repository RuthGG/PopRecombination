...
$ENVPY code/python/makeIsochores.py -f data/genomicSuperDups_coords.bed -t 0.1  -o "analysis/$(date +'%Y%m%d')_LocationPatterns" -d genomicSuperDups
...
for SEGMENT in $(ls $FILESDIR/Segments/*); do
  SEGNAME=$(echo $SEGMENT | rev | cut -d'/' -f 1 | rev | sed "s/\.bed//g")
  for NAME in $CONDITIONS; do
    /home/rgomez/anaconda3/bin/gat-run.py --segments=$SEGMENT --annotations=$FILESDIR/Annotations/annotations_${NAME}.bed --output-counts-pattern=${OUTDIR}/counts/${SEGNAME}_${NAME}_%s.tsv.gz --isochores=$FILESDIR/Isochores/genomicSuperDups_COzones_0.1/windows.txt --workspace=$FILESDIR/Workspace/workspace_${NAME}.bed --num-samples=100 --log=$OUTDIR/log/${SEGNAME}_${NAME}.log  --stdout=$OUTDIR/tsv/${SEGNAME}_${NAME}.tsv
...
# Analysis: are invesions in regions with lower recombination?

```
python  code/python/invRecValues.py -i data/InversionsAnnotation_131Inv_20211117.csv -l data/SpenceSong_hg19_recMaps_processed/CEU_recombination_map_hg19_allChr.bed -o "analysis/20220302_LocationPatterns/"
```
...
chrRates<-read.table("analysis/20220302_LocationPatterns/20220311_invrecValues/chromRates.csv", header = T)

ggplot(chrRates, aes(x = as.factor(Chr))) +
  geom_boxplot(aes( 
      lower = mean - std, 
      upper = mean + std, 
      middle = mean, 
      ...
      ymax = mean + 3*std),
    stat = "identity")+
  geom_jitter(data = invRates, aes(x = Chr, y = inside_WMR, color = OriginFixed), alpha = 0.7, width = 0.25)+
  geom_text(data = invRates[invRates$inside_WMR > 2e-07,],aes(x = Chr, y = inside_WMR, color = OriginFixed, label = INV), hjust = -0.3 )+
  scale_y_log10()
```
