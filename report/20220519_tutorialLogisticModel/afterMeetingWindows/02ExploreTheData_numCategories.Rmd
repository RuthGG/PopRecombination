...
winRegions<-winRegions[,c("Chromosome", "Start", "End", "ChrRegion", "invCenters", "NHCenters", "NAHRCenters", "Length.Mb", "allRepCounts", "WAvgRate.perMb" )]
...
I show the data distributions in the Figure \@ref(fig:varDistribution). The inversion counts have only a number of possible options, so they can be considered an ordinal variable. Since there are only a few cases of some of the inversion count options (Table \@ref(tab:reassignCategoriesB)) I will make a "3 or more" category (count cases in Table \@ref(tab:reassignCategoriesA)). The independent variables are continuous and categorical, so assumptions 1 and 2 are satisfied. 



```{r varDistribution, eval=runR, echo=showcode,  fig.cap="Raincloud plots for each variable.",  fig.height=5.5 }

pacman::p_load(ggdist, ggplot2, gghalves, reshape2, patchwork)
  
windowData <- winRegions
  # Put chromosome factor in order
  windowData$Chromosome <- factor(windowData$Chromosome, levels = paste(rep("chr", 23), as.character(c(1:22, "X")), sep = ""))
  # windowData$chromID <- windowData$winID <- NULL
  
  # Melt data
  windowDataMelted <- melt(windowData, id.vars = c( "Chromosome", "Start", "End", "ChrRegion", "ChromType")) 
...
```{r reassignCategoriesB, echo=showcode, eval=runR}
...
```{r reassignCategoriesA, echo=showcode, eval=runR}
...
```{r groupDifferences, eval=runR, echo=showcode,  fig.cap="Boxplots for each dependent variable group and each independent variable quickly show candidates of having a strong effect. We can also see that there is missing data for some chromosome region types, because windows with 3+ inversions are scarce.",  fig.height=8 }

pacman::p_load(ggplot2, reshape2,  grid, gridExtra)

winRegions_long<-melt(winRegions, id.vars = c("Start", "End", "Chromosome", "invCenters", "NHCenters", "NAHRCenters", "invCategory", "NHCategory", "NAHRCategory","ChrRegion", "ChromType"))
...
```{r correlations, eval=runR, echo=showcode,  fig.cap="Correlation matrix of independent variables.",  fig.height=4 }
pacman::p_load(PerformanceAnalytics, GGally, cowplot)

# Select columns
cols<- c("Length.Mb", "allRepCounts", "WAvgRate.perMb", "ChrRegion")
...
# Setup corrplot
 ...
 p1<- ggpairs(winRegions,  mapping = aes(color = ChrRegion, alpha = 0.5),columns = cols,
  ...
  p2<- ggpairs(winRegions,  mapping = aes(color = ChrRegion, alpha = 0.5),columns = cols,
              # lower = list(continuous = wrap("smooth", size = 0.5)),
              lower = list(continuous = lines),
              diag = list(continuous = diagonal),
...
if ("chrX" %in% winRegions$ChromType){
  modlm <- lm(as.numeric(as.character(invCenters)) ~ Length.Mb + allRepCounts + ChrRegion + WAvgRate.perMb + ChromType , data = winRegions)
knitr::kable(vif(modlm), caption = "Variance Inflation Factor")
}else{
  modlm <- lm(as.numeric(as.character(invCenters)) ~ Length.Mb + allRepCounts + ChrRegion + WAvgRate.perMb , data = winRegions)
knitr::kable(vif(modlm), caption = "Variance Inflation Factor")
...
knitr::kable(as.data.frame(apply(winRegions[,c("Length.Mb", "Length.Mb.Scaled")],2,summary)), caption = "Example of variable scalation.")
```


```{r descaleVars, echo = FALSE, eval=FALSE}
