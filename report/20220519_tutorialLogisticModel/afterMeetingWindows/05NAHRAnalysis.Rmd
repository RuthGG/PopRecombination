...
s<-summary(as.numeric(as.character(NAHRCenters)) ~ Length.Mb + allRepCounts +Color + WAvgRate.perMb, data=winRegions, fun=sf)

# Transform to dataframe
df<-data.frame(unclass(s))
df$categValues<-rownames(df)
df$categories<-c(rep("Length.Mb",4), rep("allRepCounts",4), rep("Color",3), rep("WAvgRate.perMb", 4), "Total")

# Melt dataframe
mdf<-melt(df, id.vars = c("categValues", "categories", "N"))
levels(mdf$variable)<-c(">=0", "0 vs 1+2", "0+1 vs 2")
...
Length_sim<-seq(min(winRegions$Length.Mb), max(winRegions$Length.Mb)*2, length.out = 50)
allRepCounts_sim<-seq(min(winRegions$allRepCounts), max(winRegions$allRepCounts)*2, length.out = 4)
WAvgRate.perMb_sim<-seq(min(winRegions$WAvgRate.perMb), max(winRegions$WAvgRate.perMb)*2, length.out = 4)
Color_sim = c("centromeric", "telomeric", "arm")

# Make all comination for all values
data_sim<-expand.grid(Length_sim, allRepCounts_sim, WAvgRate.perMb_sim, Color_sim)
colnames(data_sim)<-c("Length.Mb", "allRepCounts", "WAvgRate.perMb","Color")

# Calculate probabilities for each case
data_sim <- cbind(data_sim, predict(mod, data_sim, type = "probs"))

# Make plot
data_plot <- melt(data_sim, id.vars = c("Length.Mb", "allRepCounts", "Color", "WAvgRate.perMb"),value.name="Probability", variable.name = "Level")
...
ggplot(data_plot) +geom_line(aes(x = Length.Mb, y = Probability, group = interaction(Level, allRepCounts), color=Level, alpha  = allRepCounts))+
  facet_grid(WAvgRate.perMb~Color,labeller = "label_both" )+
