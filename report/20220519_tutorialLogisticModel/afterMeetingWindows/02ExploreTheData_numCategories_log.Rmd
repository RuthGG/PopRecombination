...
knitr::kable(alltab, caption = "New counts")
...
winRegions_long<-melt(winRegions[, which(!(colnames(winRegions) %in% c("ChromType", "allRepCounts")))], id.vars = c("Start", "End", "Chromosome", "invCenters", "NHCenters", "NAHRCenters", "invCategory", "NHCategory", "NAHRCategory","Color"))
...
# Select columns
cols<- c("Length.Mb", "log10RepCounts", "WAvgRate.perMb", "Color")
...
if ("chrX" %in% winRegions$ChromType){
  modlm <- lm(as.numeric(as.character(invCenters)) ~ Length.Mb + log10RepCounts + Color + WAvgRate.perMb+ChromType , data = winRegions)
knitr::kable(vif(modlm))
}else{
  modlm <- lm(as.numeric(as.character(invCenters)) ~ Length.Mb + log10RepCounts + Color + WAvgRate.perMb , data = winRegions)
knitr::kable(vif(modlm))
