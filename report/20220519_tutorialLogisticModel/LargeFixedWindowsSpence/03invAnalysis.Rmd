...
s<-summary(as.numeric(as.character(invCenters)) ~ Length.Mb + allRepCounts +Color + WAvgRate.perMb, data=winRegions, fun=sf)

# Transform to dataframe
df<-data.frame(unclass(s))
df$categValues<-rownames(df)
df$categories<-c(rep("Length.Mb",4), rep("allRepCounts",4), rep("Color",2), rep("WAvgRate.perMb", 4), "Total")
...
ggplot(data_plot) +geom_line(aes(x = Length.Mb, y = Probability, group = interaction(Level, WAvgRate.perMb), color=Level, alpha  = WAvgRate.perMb))+
  facet_grid(allRepCounts~Color,labeller = "label_both" )+
  scale_color_manual(values = c("#440154","#31688e","#35b779" , "#c8e020"))+
  ggtitle(paste0("Probability of inversion level (",yVar,") for multiple scenarios"))+
  theme(legend.position = "top",legend.box="vertical")
```