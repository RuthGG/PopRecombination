...
s<-summary(as.numeric(as.character(NAHRCenters)) ~ Length.Mb.Scaled + allRepCounts.Scaled +Color + WAvgRate.perMb.Scaled, data=winRegions, fun=sf)
...
Length_sim<-seq(min(winRegions$Length.Mb.Scaled), max(winRegions$Length.Mb.Scaled)*2, length.out = 50)
allRepCounts.Scaled_sim<-seq(min(winRegions$allRepCounts.Scaled), max(winRegions$allRepCounts.Scaled)*2, length.out = 4)
WAvgRate.perMb.Scaled_sim<-seq(min(winRegions$WAvgRate.perMb.Scaled), max(winRegions$WAvgRate.perMb.Scaled)*2, length.out = 4)
