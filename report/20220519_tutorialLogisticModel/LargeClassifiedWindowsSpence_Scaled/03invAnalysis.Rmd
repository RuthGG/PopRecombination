...
s<-summary(as.numeric(as.character(invCenters)) ~ Length.Mb.Scaled + allRepCounts.Scaled + Color + WAvgRate.perMb.Scaled , data=winRegions, fun=sf)
...
allRepCounts_sim<-seq(min(winRegions$allRepCounts.Scaled), max(winRegions$allRepCounts.Scaled), length.out = 4)
WAvgRate.perMb_sim<-seq(min(winRegions$WAvgRate.perMb.Scaled), max(winRegions$WAvgRate.perMb.Scaled), length.out = 4)
